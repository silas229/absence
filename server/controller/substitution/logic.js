        ...
        client.fetch().then((data) => {
            ...
            return new Promise(resolve => {
                ...
                $("table:not(.info):not(.mon_head) tr:not(:has(>th))").each(function(index) {
                    ...
                    if(element[0].includes(class_id)){result_buffer.push({
                            hour: replacews(element, 1),
                            "teacher": replacews(element, 2).replace('<td class="list">', ''),
                            "old_lesson": replacews(element, 3),
                            "lesson": replacews(element, 4),
                            "room": replacews(element, 5)});}
                });
                resolve(result_buffer);
            });
        }).then(dat => resolvef(dat));
    });
};
